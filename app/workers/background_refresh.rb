class BackgroundRefresh
  ...
  def perform(user_id)
    ...
    # ActiveRecord::Base.connection_pool.with_connection do
      ...
      unless new_top_post_date == old_top_post_date
        ...
        # num_of_new_messages == 1 ? subject_verb_agreement_prefix = "is" : subject_verb_agreement_prefix = "are"
        # num_of_new_messages == 1 ? correct_pluralization_postfix = "post" : correct_pluralization_postfix = "posts"
        # num_of_posts_text_prefix  = "There #{subject_verb_agreement_prefix} "
        # num_of_posts_text_postfix = " new #{correct_pluralization_postfix}"

        # need postfix generated by JS, not rb
  ...
  def posts_html(posts)
    ...
    posts.each do |post|
      html +=
      "<div class=\'panel panel-default new_post\' style=\'display: none\'><div class=\'panel-heading\'> <a href=\'#{post[:user_url]}\' target=\'_blank\'>   <img href=\'#{post[:user_image_url]}\' style=\'float: left; padding-right: 10px; max-width:50px; max-height:50px;\'>   <strong>#{post[:user_screen_name]}</strong><br>"
